...
function check(over: Record<string, unknown> = {}) {
  ...
  const result = spawnSync(process.execPath, ["--experimental-strip-types", script, "--no-network", file], {
    encoding: "utf8",
    env: { ...process.env, CI: "", GITHUB_ACTIONS: "", HOME: dir, XDG_CONFIG_HOME: dir },
  });
  return { status: result.status, output: result.stdout + result.stderr };
}
describe("SPEC-0095 R5 cite-check shapes", () => {
  it("accepts cited aliases and candidates", () => {
    const result = check();
