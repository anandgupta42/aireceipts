...
| `agentType` | enum | `claude-code` \| `codex` \| `cursor` \| `gemini` \| `opencode` \| `unknown` | Resolved for receipt, mini, compare, benchmark, handoff, pr, backfill writes, and demo when the rendered sessions have one source. List scans, dry backfill summaries, mixed-agent results, stats, week, quota, help, version, and setup use `unknown`. |
