...
| `agentType` | enum | `claude-code` \| `codex` \| `cursor` \| `gemini` \| `opencode` \| `unknown` | Resolved source for a single-agent render. Help, version, setup, and mixed-agent renders remain `unknown`. |
...
| `agentType` | enum | `claude-code` \| `codex` \| `cursor` \| `gemini` \| `opencode` \| `unknown` | Resolved source if one agent session was loaded before the error; otherwise `unknown`. |
