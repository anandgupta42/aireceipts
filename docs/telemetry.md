...
| `agentType` | enum | `claude-code` \| `codex` \| `cursor` \| `gemini` \| `opencode` \| `unknown` | Resolved for receipt, mini, compare, benchmark, handoff, pr, backfill writes, demo, and list when every included session has one source. Dry backfill summaries, mixed-agent results, stats, week, quota, help, version, and setup use `unknown`. |
